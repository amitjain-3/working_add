...
bool deepracer_interfaces_pkg__msg__infer_results_array__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("deepracer_interfaces_pkg.msg._infer_results_array.InferResultsArray", full_classname_dest, 67) == 0);
  }
  deepracer_interfaces_pkg__msg__InferResultsArray * ros_message = _ros_message;
  {  // results
    ...
    if (!deepracer_interfaces_pkg__msg__InferResults__Sequence__init(&(ros_message->results), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create deepracer_interfaces_pkg__msg__InferResults__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    deepracer_interfaces_pkg__msg__InferResults * dest = ros_message->results.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!deepracer_interfaces_pkg__msg__infer_results__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of InferResultsArray */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("deepracer_interfaces_pkg.msg._infer_results_array");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "InferResultsArray");
  ...
  {  // results
    ...
    deepracer_interfaces_pkg__msg__InferResults * item;
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->results.data[i]);
      PyObject * pyitem = deepracer_interfaces_pkg__msg__infer_results__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
